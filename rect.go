...
func (r screenRect) draw(dst *ebiten.Image, color color.Color) {
	...
	if r.x < 0 { // left part is off the screen
		screenRect{screenWidth + r.x, r.y, -r.x, r.height}.draw(dst, color)
		screenRect{0, r.y, r.width + r.x, r.height}.draw(dst, color)
	} else if r.x+r.width > screenWidth { // right part is off the screen
		screenRect{0, r.y, r.x + r.width - screenWidth, r.height}.draw(dst, color)
		screenRect{r.x, r.y, screenWidth - r.x, r.height}.draw(dst, color)
	}

	if r.y < 0 { // upper part is off the screen
		screenRect{r.x, screenHeight + r.y, r.width, -r.y}.draw(dst, color)
		screenRect{r.x, 0, r.width, r.height + r.y}.draw(dst, color)
	} else if r.y+r.height > screenHeight { // bottom part is off the screen
		screenRect{r.x, 0, r.width, r.y + r.height - screenHeight}.draw(dst, color)
		screenRect{r.x, r.y, r.width, screenHeight - r.y}.draw(dst, color)
	}
